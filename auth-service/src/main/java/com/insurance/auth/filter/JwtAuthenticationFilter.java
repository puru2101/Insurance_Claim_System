 ...
 *
 * This filter validates JWT tokens for protected endpoints within auth-service
 * (e.g., GET /auth/me, POST /auth/logout, POST /auth/change-password).
 *
 * Runs ONCE per request (OncePerRequestFilter).
 * If the token is valid, sets the authentication in the SecurityContext,
 * which Spring Security uses for authorization checks.
 */
@Slf4j
@Component
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (StringUtils.hasText(token) && jwtTokenProvider.validateToken(token)) {
            ...
            // Set in SecurityContext — now Spring Security knows this user is authenticated
